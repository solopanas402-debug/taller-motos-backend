...
@cors_enabled
@auth_required
@role_required(["ADMIN", "VENDEDOR"])  # Solo estos roles pueden consultar clientes
def lambda_handler(event, context):
    ...
    try:
        ...
        print(f"Consultando clientes - Página: {page}, Límite: {limit}, Búsqueda: {search}")
        ...
        print(f"Consulta exitosa - Encontrados: {len(result.get('data', result)) if isinstance(result, dict) else len(result)} clientes")
        ...
        return ResponseUtils.internal_server_error_response(
            "Ha ocurrido un problema interno al consultar los clientes"
        )